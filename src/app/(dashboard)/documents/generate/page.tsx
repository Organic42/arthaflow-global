...
export default function AIDocGeneratorPage() {
  ...
  function printDocument() {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>${doc.title}</title>
      ...
      <div class="lbl">${docLabels[previewType]}</div>
      <h1>${doc.title}</h1>
      ${doc.subtitle ? `<div class="sub">${doc.subtitle}</div>` : ""}
      ${doc.reference ? `<div class="ref">Ref: ${doc.reference}</div>` : ""}
      <hr/>
      ${doc.sections
        .map((s) => {
          let inner = "";
          if (s.type === "kv")
            inner = `<table>${(s.rows ?? [])
              .map((r) => `<tr><td class="l">${r.label}</td><td class="v">${r.value}</td></tr>`)
              .join("")}</table>`;
          if (s.type === "text") inner = `<p>${s.body ?? ""}</p>`;
          if (s.type === "list")
            inner = `<ul>${(s.items ?? []).map((i) => `<li>${i}</li>`).join("")}</ul>`;
          return `<h2>${s.heading}</h2>${inner}`;
        })
        .join("")}
      ${doc.footer ? `<div class="footer">${doc.footer} · Generated by ArthaFlow Global</div>` : ""}
      </body></html>`);
    win.document.close();
    win.focus();
    win.print();
