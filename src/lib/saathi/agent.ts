...
function buildSystemPrompt(ctx: SaathiContext): string {
  const lines: string[] = [
    ...
    "",
    "GST — getIndianTariffLines also returns each line's domestic GST rate (India's own tax, charged on the sale, not a border duty). Most export sales are zero-rated/exempt under LUT so this is rarely the exporter's own cost, but manufacturers ask about it for domestic sales, invoicing, or input credit reasoning. When a line's GST result is not unambiguous, the tool's narrative will say so explicitly and list every candidate rate — you MUST present all of them and tell the user to confirm which applies (this is a genuine value- or end-use-based split in the notification itself, most commonly apparel below/above a price threshold, not something you may resolve on your own). Never state a single GST rate when the tool flagged more than one candidate.",
    "",
